/**
 * Copyright (C) 2014 yanni4night.com
 *
 * test-cookie.js
 *
 * changelog
 * 2014-06-08[16:55:18]:authorized
 * 2014-06-13[10:33:59]:test cookie&removeCookie
 *
 * @author yanni4night@gmail.com
 * @version 0.1.1
 * @since 0.1.0
 */

(function(window, document, undefined) {
    var assert = require('assert');
    var cookie = require('../cookie');
    var PassportCookieParser = cookie.PassportCookieParser;

    describe('Cookie', function() {
        describe('#PassportCookieParser', function() {
            it('should parse userid', function() {
                if (navigator.cookieEnabled) {
                    //sohu cookie
                    document.cookie = 'ppinf=2|1402199707|1403409307|';
                    assert.equal('BF56C74E50C5295E4600B4A444AC31A0@qq.sohu.com', PassportCookieParser.parsePassportCookie().getCookie().userid);
                    //sogou cookie
                    document.cookie = 'ppinf=5|1409662939|1410872539|Y2xpZW50aWQ6NDoyMDAyfGNydDoxMDoxNDA5NjYyOTM5fHJlZm5pY2s6MDp8dHJ1c3Q6MToxfHVzZXJpZDoxNjpsb3ZlbWRAc29nb3UuY29tfHVuaXFuYW1lOjA6fA';
                    assert.equal('lovemd@sogou.com', PassportCookieParser.parsePassportCookie().getCookie().userid);
                }
            });
        });

        describe('#cookie()',function(){
            var expando = 'sogou-passport-test-cookie-cookie'+ (+new Date());
            cookie.cookie(expando,1);
            it('should make a cookie',function(){
                assert.equal(1,cookie.cookie(expando));
            });
        });
        describe('#removeCookie()',function(){
            var expando = 'sogou-passport-test-cookie-removeCookie'+ (+new Date());
            cookie.cookie(expando,1);
            it('should remove a cookie',function(){
                assert(cookie.removeCookie(expando));
            });
        });

    }); //describe
})(window, document);